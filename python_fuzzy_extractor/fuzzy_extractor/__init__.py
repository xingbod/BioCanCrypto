...
class FuzzyExtractor(object):
    ...
    def generateKey(self):
        key = np.fromstring(urandom(self.length), dtype=np.uint8)  # key should be far to each other
        return key
    ...
    def generateWithPreGeneratedkey(self, value,key):# this one comes with defined key
        """Takes a source value and produces a key and public helper
