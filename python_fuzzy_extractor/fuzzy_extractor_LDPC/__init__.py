...
class FuzzyExtractorLDPC(object):
    ...
    def __init__(self, length, ham_err, rep_err=0.0001, **locker_args):
        ...
        self.lDPC_FE_Util = LDPC_FE_Util(128, 3, 16)  # message 144 bits, encode into 128 bits, which corespond to fuzzy extractor 16 uint8 (128bits)
    ...
    def generateKey(self):
        key_gen, keys_int = self.lDPC_FE_Util.genKey()
        return key_gen
    ...
    def generate(self, value):
        ...
        key_gen, keys_int = self.lDPC_FE_Util.genKey()
        key_ec, key_ec_int = self.lDPC_FE_Util.encode(key_gen)
        key_pad = np.concatenate((key_ec, np.zeros(self.sec_len*8, dtype=np.uint8)))

        nonces = np.zeros((self.num_helpers, self.nonce_len), dtype=np.uint8)
        masks = np.zeros((self.num_helpers, self.length), dtype=np.uint8)
        digests = np.zeros((self.num_helpers, self.cipher_len*8), dtype=np.uint8)
        ...
        for helper in range(self.num_helpers):#18
            ...
            digests[helper] = np.array([self.lDPC_FE_Util.bin_array(k, 8) for k in digest]).reshape(-1)

        # print(np.shape(digests))#(42591,18)
        # print(np.shape(key_pad))#(18)
        ciphers = np.bitwise_xor(digests, key_pad)

        return (key_gen.tobytes(), (ciphers, masks, nonces))

    def generateWithPreGeneratedkey(self, value, key_gen):# comes with a key
        """Takes a source value and produces a key and public helper
    ...
    def reproduce(self, value, helpers):
        ...
        # the locker.
        checks = np.sum(plains[:, -self.sec_len*8:], axis=1)
        for check in range(self.num_helpers):
            if checks[check] == 0:
                ec = plains[check, :-self.sec_len*8]
                decode_key, de_key_int = self.lDPC_FE_Util.decode(ec)
                return decode_key.tobytes()
